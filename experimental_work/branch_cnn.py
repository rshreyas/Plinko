...
def Branch_CNN_Classifier(x_train, y_train, x_test, y_test, taxid, n_gram, taxon_level):
	...
	conv_0_1 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[0], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)
        conv_1_1 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[1], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)
        conv_2_1 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[2], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)

        maxpool_0_1 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[0] + 1, 1), strides=(1,1), padding='valid')(conv_0_1)
        maxpool_1_1 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[1] + 1, 1), strides=(1,1), padding='valid')(conv_1_1)
        maxpool_2_1 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[2] + 1, 1), strides=(1,1), padding='valid')(conv_2_1)

        concatenated_tensor = Concatenate(axis=1)([maxpool_0_1, maxpool_1_1, maxpool_2_1])
        flatten = Flatten()(concatenated_tensor)
	reshape_1 = Reshape((-1, 1))(flatten)
	
	#-------------------------#
        # Coarse Branch 1 (ORDER) #
        #-------------------------#

	c_1_bch = Dense(64, activation='relu', name='c1_fc1')(flatten)
	...
	conv_0_2 = Conv1D(200, 6, padding='valid', activation='relu')(reshape_1)
        bnorm_0_2 = BatchNormalization(scale = False)(conv_0_2)
	mpool_0_2 = MaxPooling1D(strides = 2)(bnorm_0_2)
	conv_1_2 = Conv1D(200, 6, padding='valid', activation='relu')(mpool_0_2)
        bnorm_1_2 = BatchNormalization(scale = False)(conv_1_2)
	mpool_1_2 = MaxPooling1D(strides = 2)(bnorm_1_2)
	conv_2_2 = Conv1D(200, 6, padding='valid', activation='relu')(mpool_1_2)
        bnorm_2_2 = BatchNormalization(scale = False)(conv_2_2)
	mpool_2_2 = MaxPooling1D(strides = 2)(bnorm_2_2)
	
	#--------------------------#
        # Coarse Branch 2 (FAMILY) #
        #--------------------------#

	c_2_bch = Flatten(name='c2_flatten')(mpool_2_2)
	...
	conv_0_3 = Conv1D(200, 6, padding='valid', activation='relu')(bnorm_2_2)
        bnorm_0_3 = BatchNormalization(scale = False)(conv_0_3)
	#mpool_0_3 = MaxPooling1D(strides = 2)(bnorm_0_3)
        conv_1_3 = Conv1D(200, 6, padding='valid', activation='relu')(bnorm_0_3)
        bnorm_1_3 = BatchNormalization(scale = False)(conv_1_3)
	#mpool_1_3 = MaxPooling1D(strides = 2)(bnorm_1_3)
        conv_2_3 = Conv1D(200, 6, padding='valid', activation='relu')(bnorm_1_3)
        bnorm_2_3 = BatchNormalization(scale = False)(conv_2_3)
	mpool_2_3 = MaxPooling1D(strides = 2)(bnorm_2_3)

	#--------------------------#
        # Fine Predictions (GENUS) #
        #--------------------------#
	
	fine_bch = Flatten(name='flatten')(mpool_2_3)
...
def create_hdf5_datasets(path_to_test_train_dir):
	...
	f = h5py.File(path_to_test_train_dir + "28Genus_" + strftime('%X') + ".hdf5", "w")
...
if __name__ == '__main__':
	...
	call_branch_cnn('./inputs/simple_dataset_raw/')
