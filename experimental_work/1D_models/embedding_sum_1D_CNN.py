...
from keras.layers import Convolution1D, Conv1D, Conv2D, Lambda, SpatialDropout1D, MaxPooling1D, MaxPooling2D, MaxPool2D, LSTM, Bidirectional, TimeDistributed
...
if __name__ == '__main__':
	...
	#	for directory in os.listdir(directory_for_training):
			#print"Classifying at taxa level {} for TaxID: {}".format(taxa_rank, directory)
                x_train, y_train = dataset_load(directory_for_training, "1578")
                x_test, y_test = dataset_load(directory_for_testing, "1578")
                filter_width = 15

                CNNClassifier(x_train, y_train, x_test, y_test, "1578", 6, taxa_rank, filter_width)
	'''
