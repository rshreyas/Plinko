...
def CNNClassifier(x_train, y_train, x_test, y_test, taxid, n_gram, taxon_level, filter_size):
	...
	FILTER_SIZES = [3 , filter_size - KMER_LEN + 1, 5]
	...
	conv_0 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[1], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)
	conv_1 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[1], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)
	conv_2 = Conv2D(NUM_FILTERS, kernel_size=(FILTER_SIZES[1], EMBEDDING_DIM), padding='valid', kernel_initializer='normal', activation='relu')(reshape)

	maxpool_0 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[1] + 1, 1), strides=(1,1), padding='valid')(conv_0)
	maxpool_1 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[1] + 1, 1), strides=(1,1), padding='valid')(conv_1)
	maxpool_2 = MaxPool2D(pool_size=(KMER_COUNT - FILTER_SIZES[1] + 1, 1), strides=(1,1), padding='valid')(conv_2)
 
	concatenated_tensor = Concatenate(axis=1)([maxpool_0, maxpool_1, maxpool_2])
	
	reshape_2 = Reshape((NUM_FILTERS, -1))(concatenated_tensor)
...
def deepexplain_call(x_test, y_test):
	with DeepExplain(session=K.get_session()) as de:
		model = load_model("./tree_cnn_model_store/genus/1578/100FragmentLength_6mer_genus_1578_15.h5")
		print "Model Loaded.."
		...
		print("pre_softmax_tensor --- {} ".format(output_tensor))
		
		###############################################
		# Get X_test and Y_test in the correct format #
		###############################################
		word_index = create_embedding_matrix(6, predict = True)
		index2word = {v: k for k,v in word_index.items()}
		start_at = random.randint(0, len(x_test) - 20)
		end_at = start_at + 20

		xs = generate_next_batch(x_test, start_at, end_at, word_index)

        	mapped_classes, num_classes_y = map_to_0_index(np.unique(y_test))
        	mapped_to_taxid = map_back_to_taxid(mapped_classes)
        	y_test = y_test.flatten()
        	ys = y_test[start_at:end_at]
		ys = [mapped_classes[label] for label in ys]
		ys = keras.utils.to_categorical(ys, num_classes_y)
		################################################
		
		###############################################
		# Create new model with output as pre-softmax #
		###############################################

		sess = K.get_session()
		embedding_out = sess.run(embedding_tensor, {input_tensor: xs})		
		fModel = Model(inputs = input_tensor, outputs = output_tensor)

		attributions = de.explain('elrp', output_tensor * ys, fModel.layers[1].output, embedding_out)
		
		for idx, a in enumerate(attributions):
			words = [index2word[i] for i in xs[idx]]
			attribution_sum = np.sum(a, axis = 0)
			#norm = [float(i)/sum(attribution_sum) for i in attribution_sum]
			j_words = '\t'.join(words)
			j_attribution_sum = '\t'.join(map(lambda x: str(x), attribution_sum))
			with open("visualize_layerwise_relevance.csv", "a+") as fopen:
				fopen.write(j_words + "\n")
				fopen.write(j_attribution_sum + "\n")	
...
def tensorboard_viz():
	...
	place = tf.placeholder(tf.float32, shape=[embedding_matrix.shape[0], embedding_matrix.shape[1]])
	set_x = tf.assign(X, place, validate_shape=False)

	sess.run(tf.global_variables_initializer())
	sess.run(set_x, feed_dict={place: embedding_matrix})
	with open(metadata_path, 'w') as f:
		for word, index in word_index.items():
			line_out = "%s\n" % word
			f.write(line_out.encode('utf-8'))

	summary_writer = tf.summary.FileWriter(LOG_DIR + '/' + 'emb_viz_1.log', sess.graph)
